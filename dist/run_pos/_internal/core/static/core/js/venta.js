let ordenOriginalCategorias = [];
        ...
        .then(data => {
            if (data.es_superusuario) {
                ...
                if (confirm("¿Estás seguro de que deseas reabrir esta comanda?")) {
                    ...
                    .catch(error => {
                        console.error("Error al reabrir la comanda:", error);
    ...
    function imprimirComandaDesdeModal(tipo = 'cliente') {
        // Obtén el ID de la comanda desde el contenido del modal
        ...
        const comandaId = match[1];

        // Determinar la URL según el tipo de impresión
        const url = tipo === 'cocina' 
            ? `/imprimir-boleta-cocina/${comandaId}/`
            : `/imprimir-boleta-comanda/${comandaId}/`;
        ...
        .then(data => {
            if (data.status === 'ok') {
                const tipoTexto = tipo === 'cocina' ? 'boleta de cocina' : 'boleta de cliente';
                alert(`${tipoTexto.charAt(0).toUpperCase() + tipoTexto.slice(1)} enviada a la impresora.`);
            } else {
                alert('Error al imprimir la boleta: ' + (data.message || ''));
            }
        })
        .catch(() => {
