...
function enviarEdicion(event) {
    ...
    fetch(`/editar_usuario/${id}/`, {
        ...
        .catch((error) =>
            showAppModal("Error al actualizar usuario: " + error, {
                variant: "danger",
                title: "Error al actualizar",
            }),
        );
...
function confirmarCambioEstado(userId, username, activar) {
    ...
    document.getElementById("textoConfirmacionEstado").textContent = `¿Estás seguro que deseas ${accion} al usuario "${username}"?`;
